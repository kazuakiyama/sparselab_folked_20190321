...
def imaging(
        initimage, Nuvs, Nz=1,
        imagewin=None,
        fcvtable=None,amptable=None, bstable=None, catable=None,
    ...
    dofluxconst = False
    if ((fcvtable is None) and (amptable is None) and (totalflux is None)):
        ...
        return -1
    elif ((fcvtable is None) and (amptable is None) and
    ...
    if totalflux is None:
        ...
        if fcvtable is not None:
            totalflux.append(fcvtable["amp"].max())
    ...
    if dofluxconst:
        ...
        fcvtable = pd.concat([totalfluxdata, fcvtable], ignore_index=True)
    else:
    ...
    '''
    outimage = np.zeros(Nz, dtype=np.float64)
    for z in range(Nz):
        outimage[z] = copy.deepcopy(initimage)
        outimage[z].data[istokes, ifreq] = 0.
        for i in np.arange(len(xidx)):
            outimage[z].data[istokes, ifreq, yidx[i] - 1, xidx[i] - 1] = Iout[i,z]
        outimage[z].update_fits()
    '''
    return outimage
